...
CSV_FILE = "neon-integer-stats.csv"


def plot_throughput(csv_path):
    """
    Reads and plots the throughput data from the integer NEON benchmark CSV.
    """
    if not os.path.exists(csv_path):
        print(f"Error: Benchmark data file not found at '{csv_path}'")
        print(
            "Please run the benchmark first: ./target/release/neon-integer --output neon-integer-stats.csv"
        )
        return
    ...
    ax.plot(df["core_id"], df["throughput"], marker="o", linestyle="-", color="green")
    ax.set_xlabel("Core ID")
    ax.set_ylabel(f"Throughput ({unit})")
    ax.set_title("Integer NEON SIMD Throughput per Core (Add Operations)")
    ax.grid(True)
    ax.set_xticks(df["core_id"])

    output_path = "neon-integer-throughput.png"
