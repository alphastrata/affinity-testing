...
// Use 8 operations per SIMD loop for advanced ARM NEON (2x4 f32 lanes)
const SIMD_LOOPS: u64 = 250_000_000; // To maintain comparable operation count

#[derive(FromArgs)]
/// Profiles advanced SIMD (NEON) math throughput on all cores for ARM64.
struct AppArgs {
    /// output file for statistics (e.g., neon-advanced-stats.csv)
    #[argh(option, short = 'o')]
    output: String,
}

#[cfg(target_arch = "aarch64")]
fn neon_advanced_workload() -> (f64, String) {
    core_profiler::simd_arm::neon_multiply_workload_advanced(SIMD_LOOPS)
}
...
fn main() {
    #[cfg(target_arch = "aarch64")]
    {
        ...
        let results = profile_workload_on_all_cores(neon_advanced_workload);
