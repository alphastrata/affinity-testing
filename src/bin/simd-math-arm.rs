...
// Use 4 operations per SIMD loop for ARM NEON (4 f32 lanes)
const SIMD_LOOPS: u64 = 500_000_000; // Increased loops to compensate for fewer lanes

#[derive(FromArgs)]
/// Profiles SIMD (NEON) math throughput on all cores for ARM64.
struct AppArgs {
    /// output file for statistics (e.g., simd-math-arm-stats.csv)
    #[argh(option, short = 'o')]
    output: String,
}
