...
// Use 4 operations per SIMD loop for integer ARM NEON (4 s32 lanes)
const SIMD_LOOPS: u64 = 500_000_000; // Increased loops to compensate for fewer lanes

#[derive(FromArgs)]
/// Profiles integer SIMD (NEON) math throughput on all cores for ARM64.
struct AppArgs {
    /// output file for statistics (e.g., neon-integer-stats.csv)
    #[argh(option, short = 'o')]
    output: String,
}

#[cfg(target_arch = "aarch64")]
fn neon_integer_workload() -> (f64, String) {
    core_profiler::simd_arm::neon_integer_workload(SIMD_LOOPS)
}
...
fn main() {
    #[cfg(target_arch = "aarch64")]
    {
        ...
        let results = profile_workload_on_all_cores(neon_integer_workload);
