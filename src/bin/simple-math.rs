...
#[derive(FromArgs)]
/// Profiles simple floating-point math throughput on all cores.
struct AppArgs {
    /// output file for statistics (e.g., simple-math-stats.csv)
    #[argh(option, short = 'o')]
    output: String,
}

fn simple_math_workload() -> (f64, String) {
...
fn main() {
    ...
    let results = profile_workload_on_all_cores(simple_math_workload);
